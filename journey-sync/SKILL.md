...
chain:
  lanes:
    greenfield: { position: 6, prev: spec-ac-sync, next: writing-ux-design }
    brownfield-feature: { position: 4, prev: writing-spec, next: writing-technical-design }
    drift: { position: 2, prev: spec-code-sync, next: work-item-sync }
  progressive: true
  self_verify: true
...
- In greenfield lane: `writing-ux-design --progressive --lane greenfield`
- In brownfield-feature lane: `writing-technical-design --progressive --lane brownfield-feature`
- In drift lane: `work-item-sync --progressive --lane drift`

**If `--progressive` flag is absent:**
- Report results to user
- Suggest: "Next: consider running `writing-ux-design`" (greenfield), "`writing-technical-design`" (brownfield-feature), or "`work-item-sync`" (drift)

...
chain:
  lanes:
    greenfield: { position: 4, prev: validate-feature, next: audit-ac }
...
This skill produces DRAFT specs. It defines WHAT we're building and HOW it's experienced — never HOW to implement it. UX design comes next in design-ux.
...
| Feature Ship Brief | From validate-feature | Optional | Can work from brainstorming output or user description |
...
- **convert:** Read existing specs first. Match their format, header fields, and conventions. Don't force svc conventions on first pass — adapt.
...
If the repo is brownfield and has not yet been mapped into svc working mode, route to `onboard-repo` first.
...
- In greenfield lane: `audit-ac --progressive --lane greenfield`
- In brownfield-feature lane: `write-journeys --progressive --lane brownfield-feature`

**If `--progressive` flag is absent:**
- Report results to user
- Suggest: "Next: consider running `audit-ac`" (greenfield) or "`write-journeys`" (brownfield-feature)

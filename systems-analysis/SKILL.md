...
description: >
  ...
  Runs after executing-change-set and review-protocol, verifies the implementation
...
outputs:
  produces:
    - { path: "docs/specs/audit/<name>-analysis.md", artifact: systems-analysis-report }
chain:
  lanes:
    greenfield: { position: 14, prev: executing-change-set, next: landing-change-set }
    brownfield-feature: { position: 11, prev: review-protocol, next: landing-change-set }
    bugfix: { position: 5, prev: review-protocol, next: landing-change-set }
    refactor: { position: 5, prev: review-protocol, next: landing-change-set }
  progressive: true
  self_verify: true
...
Derived from petekp/claude-code-setup exhaustive-systems-analysis (MIT,
Copyright 2024 Pete Petrash). Adapted for vibomatic's spec-first pipeline
with AC-traced verification.

**Announce at start:** "I'm using systems-analysis to audit the implementation for correctness before landing."
...
When subagents are available, assign one subsystem per subagent with disjoint files.
...
- If `--progressive`: invoke `landing-change-set --progressive --lane <lane>`
- If not progressive: suggest "Ready to land. Run `landing-change-set`"

**If verdict is BLOCKED:**
- Stop the chain. Report Critical findings.
- Route back to `executing-change-set` for fixes in the worktree.
- After fixes: re-run `systems-analysis`.

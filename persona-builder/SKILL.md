...
description: >
  ...
  skills consume (journey-sync, copywriting, page-cro, onboarding-cro, etc). Seven modes:
  ...
  proactively before journey-sync, copywriting, or any user-facing skill when
...
relationship with the platform. This is the most important section for journey-sync
...
- **journey-sync:** [what this persona's journey should emphasize, what alternative paths matter]
...
|-------|--------------|----------------|
| journey-sync | Does it use persona files? Does each journey specify which persona it serves? | Generates generic journeys that don't differentiate user types |
...
- **Specificity over generality.** "This skill should consider personas" is useless.
  "The journey-sync skill's Phase 2 builds personas from scratch instead of reading
...
- Skill Implications (does the deeper understanding change guidance for journey-sync?)
...
are direct inputs from journey-sync that tell you exactly where persona gaps exist. Journey-sync
often discovers persona gaps before persona-builder does, because it traces data flow across
...
- **Concept fragmentation findings** — if journey-sync found two names for
  the same entity, the producing role may need a persona that understands both
  the creation and consumption sides

This analysis is the highest-signal input because journey-sync already did the
cross-role tracing work. It directly tells you which roles are missing persona
coverage. Don't re-derive what journey-sync already found — read it and act on it.
...
- Invoke next skill: `feature-discovery --progressive --lane greenfield`

**If `--progressive` flag is absent:**
- Report results to user
- Suggest: "Next: consider running `feature-discovery`"

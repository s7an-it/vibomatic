...
description: >
  ...
  after vision-sync in progressive mode. Also invoked by other skills when they need
...
**Announce at start:** "I'm using the domain-expert skill to build domain expertise for this project."
...
2. If research reveals a framework/library gap, invoke `skill-finder` to check for external skills
...
[Skills found via skill-finder that could help — installed or recommended]
...
Write conventions.md and testing.md with the domain-specific knowledge. These will be loaded by `executing-change-set` during code generation.

### Research Mode (invoked by other skills)

When another skill calls domain-expert with a specific question:
...
4. If research reveals a capability gap: invoke `skill-finder` to check for external skills

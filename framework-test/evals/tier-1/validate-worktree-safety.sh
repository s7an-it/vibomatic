...
# and is executable, no orphaned worktrees under .worktrees/ or /tmp/vibomatic-*.
# No LLM, <5s.
set -euo pipefail
...
# 7. No orphaned /tmp/vibomatic-test-* worktrees
for dir in /tmp/vibomatic-test-*/; do
  [[ ! -d "$dir" ]] && continue
  ERRORS+="  FAIL: orphaned temp worktree: $dir\n"
  FAIL=$((FAIL + 1))
done
# If no /tmp dirs found, that's a pass
if ! compgen -G "/tmp/vibomatic-test-*/" >/dev/null 2>&1; then
  PASS=$((PASS + 1))
fi

...
Every vibomatic skill supports the same inputs/outputs. The mode determines who drives the sequence.

| Mode | Driver | Mechanism | Control flow |
|------|--------|-----------|-------------|
| **Standalone** | User | `/writing-spec` | Human decides what's next |
...
### Progressive
User invokes with a flag: `/vision-sync --progressive`. Skill runs, succeeds, invokes next skill with same flag. Next skill checks its preconditions (do my inputs exist on disk?), runs, passes torch. Chain continues until pipeline completes or a skill fails.
...
## What Vibomatic Gets Right (That Others Don't)

The four-mode architecture supports both greenfield and managed projects:
- **Greenfield:** `/vision-sync --progressive` chains the full pipeline from zero
- **Managed project:** `/journey-sync` standalone audits current journey state
...
- [ ] `--progressive` flag not yet implemented in vibomatic skills

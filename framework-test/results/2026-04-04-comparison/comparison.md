...
**Fix applied:** writing-spec now explicitly extracts deployment modes from the vision doc and requires ACs for each mode.
...
**Fix applied:** framework-test now includes a first-time user experience test.
...
**Fix applied:** writing-change-set now requires a config section for deployment modes when the vision mentions them.
...
3. **Post-hoc spec-code-sync** — annotate ACs as RESOLVED during code generation, not after
...
| Skill | Fix | Why |
|-------|-----|-----|
| writing-spec | Added deployment mode extraction from vision doc | Modes were in vision but never became ACs |
| writing-change-set | Added mandatory config section for deployment modes | Code never implemented the modes the spec discussed |
| framework-test | Added first-time user experience test | Empty first visit wasn't caught by any existing test |

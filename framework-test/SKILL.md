...
outputs:
  produces:
    - { path: "framework-test/results/", artifact: test-results }
chain:
  lanes: {}
  progressive: false
  self_verify: false
...
LOOP:
  ...
  5.  TEST live
      - spec-code-sync against actual source files in the worktree
...
Phase 7 (writing-change-set) produces ALL 8 parts. Not just types. Not just a spec. The change set IS the codebase.
...
3. Re-run writing-change-set for that part only
...
| C6 | Feature/Enabler/Integration cascade discovers full chain | Give writing-spec a Feature. Check if journey-sync Layer 3 identifies Enabler dependencies. Check if writing-spec queues cascade specs. | At least 2 Enabler dependencies auto-discovered without manual prompting |
...
Plant 5 known errors in a feature spec. Run single-pass review. Run full review-protocol. Count detections. The test is adversarial -- it tries to break the review process.
...
Give writing-spec a Feature. Verify that Enabler dependencies are discovered automatically. The cascade must produce additional specs, not just a note saying dependencies exist.

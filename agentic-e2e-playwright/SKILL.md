...
|-------|-----------------|--------------------------|
| Journey | `docs/specs/journeys/J*` matching the ID | Run `journey-sync` to generate journey docs from feature specs and personas |
| Personas | `docs/specs/personas/P*.md` referenced by journey header | Run `persona-builder` to create persona files |
| AC checklists | `docs/specs/features/*-ac-checklist.md` for each spec in journey's **Covers** | Run `spec-ac-sync` to generate AC checklists from feature specs |
...
`spec-ac-sync {feature-name}`. If both are missing, the feature hasn't been
...
- **Skill Implications > journey-sync** — sometimes has direct testing guidance
...
output: "Feature spec `{name}.md` doesn't have the AC table format — run `spec-ac-sync`
...
- Suggest: "Next: consider running `journey-qa-ac-testing` for runtime verification of these tests"

...
chain:
  lanes:
    brownfield-conversion: { position: 1, prev: null, next: sync-work-items }
  progressive: true
  self_verify: true
...
Instead, classify current truth into the nearest svc concepts:
...
Record compatibility notes where current structure differs from svc.
...
| 2 | work-items/INDEX.md exists | `test -f docs/specs/work-items/INDEX.md` | |
| 3 | Artifact mapping completed | project-state.md has subsystem map and work-item summary | |
...
- Invoke next skill: `sync-work-items --progressive --lane brownfield-conversion`

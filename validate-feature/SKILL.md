...
chain:
  lanes:
    greenfield: { position: 3, prev: build-personas, next: write-spec }
...
| K7 | Existing solution in codebase | Context scan | The feature already exists under a different name, or a minor fix to existing code solves the same problem. |

### Builder History Weighting

If a builder profile exists (`~/.claude/builder-profile.md`), adjust kill signal
...
| 3+ | **NO-SHIP** | Reject the feature. Present the full evidence package. Offer pivot. |
...
| Small scope, obvious implementation, no design needed | **plan-changeset** then **execute-changeset** (or **/writing-plans** if external add-on) — "This is small enough to plan directly. Want me to invoke plan-changeset with the brief as input?" |
...
| Concept fragmentation or pipeline break — not a new feature | **diagnose-bug** or **plan-changeset** (or **/writing-plans** if external add-on) — "This isn't a new feature. It's a broken pipeline / naming mess. Skip the brief, identify the fix scope, then plan the implementation directly." |
...
- Implementation plan (comes from plan-changeset or external writing-plans)

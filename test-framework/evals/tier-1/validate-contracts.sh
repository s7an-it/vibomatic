...
for skill in $SKILLS; do
  ...
  while IFS= read -r ipath; do
    ...
    # - test-framework/*, package.json, tsconfig.json, e2e/*, .agents/*
    # - Descriptive pseudo-paths in parens like "(branch checkpoint commits)"
    if ! echo "$ipath" | grep -qE '^(docs/|references/|REPO_MODES|test-framework/|DESIGN|package\.json|tsconfig\.json|e2e/|\.agents/|\()'; then

#!/usr/bin/env bash
# Shared test helpers for vibomatic evals.
...
create_test_project() {
  local name="${1:-vibomatic-test}"
  local project_dir
  project_dir=$(mktemp -d "/tmp/${name}-XXXXXX")

  cd "$project_dir"
  git init -q
  git config user.email "test@vibomatic.dev"
  git config user.name "vibomatic-test"
...
# Copy vibomatic example fixtures into a test project.

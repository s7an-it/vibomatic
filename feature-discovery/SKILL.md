...
chain:
  lanes:
    greenfield: { position: 3, prev: persona-builder, next: writing-spec }
    brownfield-feature: { position: 2, prev: spec-code-sync, next: writing-spec }
  progressive: true
  self_verify: true
...
findings directly and route to the appropriate skill (`bugfix-brief` for broken behavior,
`writing-change-set` for bounded implementation planning, `journey-sync` for a missing
journey, `spec-ac-sync` for missing ACs). Don't force 8 business questions on a problem
...
| Small scope, obvious implementation, no design needed | **writing-change-set** then **executing-change-set** (or **/writing-plans** if external add-on) — "This is small enough to plan directly. Want me to invoke writing-change-set with the brief as input?" |
...
| Concept fragmentation or pipeline break — not a new feature | **bugfix-brief** or **writing-change-set** (or **/writing-plans** if external add-on) — "This isn't a new feature. It's a broken pipeline / naming mess. Skip the brief, identify the fix scope, then plan the implementation directly." |
...
- Branch execution and checkpoints (comes from executing-change-set)
...
- Invoke next skill: `writing-spec --progressive --lane greenfield`
- In brownfield-feature lane: `writing-spec --progressive --lane brownfield-feature`

**If `--progressive` flag is absent:**
- Report results to user
- Suggest: "Next: consider running `writing-spec`"

...
|---|---|---|---|
| **Vision definition** | `vision-sync` — 3 intents (create/refine/convert), 2 modes (grounded/scratch), 12-section canonical structure, evidence table, proposal-first with approval gates | None | None |
| **Persona modeling** | `persona-builder` — 7 modes: build all, skill audit, add new, expand, interview, discover gaps, tiered auto-discovery. Lifecycle progression with trust tiers. Skill Implications feed downstream | None | None |
| **Feature discovery** | `feature-discovery` — cross-validates against personas/journeys/specs before business questions. 3-tier scan (journeys→personas→code). Escape hatch for concept fragmentation. Produces Feature Ship Brief | `/office-hours` — 6 forcing questions (demand reality, status quo, specificity, wedge, observation, future-fit). Conversational, not structured cross-validation. Produces design doc | None |
...
| **Problem framing** | Part of `writing-spec` Step 2 and `feature-discovery` | `/office-hours` — startup mode vs builder mode. YC-style partner questioning | `brainstorming` — clarifying questions one at a time, but no structured framework |
...
| Capability | vibomatic | gstack | superpowers (obra) |
|---|---|---|---|
| **Journey definition** | `journey-sync` — 6 modes (bootstrap/expand/refresh/migrate/auto/tiered). Gherkin .feature.md format. Multi-feature flows with persona trust tiers | None | None |
| **Layer 3 analysis** | Built into `journey-sync` — finds contradictions, dead ends, ungrounded preconditions, concept fragmentation, missing transitions. Routes findings to other skills | None | None |
...
| **Journey-based QA** | `journey-qa-ac-testing` — 4 modes (smoke/regression/feature-ac/exploratory). Executes against live URLs. Updates QA column in AC tables. Evidence screenshots | `/qa` + `/qa-only` — real browser testing but not journey-structured. Finds bugs and fixes them with atomic commits. Auto-generates regression tests | None |
...
| **UI in technical design** | `writing-technical-design` — component tables with responsibilities. References design system (when it exists) | Design system feeds into all design skills | `brainstorming` mentions architecture/components but not design-specific |
...
| **Code quality flywheel** | spec-code-sync detects drift, spec-ac-sync ensures ACs, journey-qa-ac-testing verifies | Review → QA → fix → regression test cycle | TDD → code review → verification-before-completion cycle |

### Testing & QA

| Capability | vibomatic | gstack | superpowers (obra) |
|---|---|---|---|
| **E2E test authoring** | `agentic-e2e-playwright` — accessibility-first selectors, read component code before testing, fix app not test. Journey-based test structure | QA skills write regression tests for found bugs. `/browse` provides real browser | None — TDD skill covers unit tests only |
| **Real browser testing** | `journey-qa-ac-testing` executes against live URLs but relies on available browser tooling | `/browse` — persistent Chromium daemon, sub-second latency, cookies/tabs/localStorage persist. ARIA ref system. 3 ring buffers | None |
...
| Capability | vibomatic | gstack | superpowers (obra) |
|---|---|---|---|
| **Feature-to-marketing pipeline** | `feature-marketing-insights` — 8 modes: full scan, single feature, refresh, foundation, compact, capability combinations, quality eval, persona validation. Weighted insights with live status | None | None |
...
6. **Real browser infrastructure** — persistent daemon for journey-qa-ac-testing
...
2. **writing-spec evolution** — Feature/Enabler/Integration cascade (already done)

...
description: >
  ...
  automatically after writing-ui-design (baseline) and after executing-change-set
...
**Announce at start:** "I'm using visual-tracker to capture/compare visual state."
...
Run after `writing-ui-design` produces the first visual design, or when
...
|---------|------|-----------|
| After `writing-ui-design` | baseline | Suggested (not auto) |
| After `executing-change-set` | diff | Suggested if baseline exists |
| After `landing-change-set` | update | Suggested if intentional changes |

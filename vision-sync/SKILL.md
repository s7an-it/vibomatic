...
description: >
  ...
  proposal-first workflow aligned with Vibomatic repository modes. Use when the
...
reality and Vibomatic workflows, without mixing in PRD or implementation planning.
...
Use `references/vibomatic-vision-template.md` as canonical structure.
...
- Vision conflicts with persona assumptions: run `vision-sync` before regenerating personas.
- Major code/spec drift discovered in `spec-code-sync`: run `vision-sync mode=grounded`.
...
- Suggest: "Next: consider running `persona-builder`"

...
description: >
  ...
  after review-gate for features with new data models or integrations.
inputs:
  required:
...
### Tri-Model Orchestration (optional)

When both Codex CLI and Gemini CLI are available, run tri-model review:

```bash
# Parallel dispatch
codex -p "$(cat review-package.md)" --output-format text > codex-review.txt &
echo "$(cat review-package.md)" | gemini > gemini-review.txt &
wait
```

Claude synthesizes both outputs:
- Where all 3 models agree → high confidence finding
- Where 2 of 3 agree → medium confidence, investigate the disagreement
- Where all 3 disagree → low confidence, likely a style issue not a bug

The disagreement IS the signal. Same-model blind spots don't show up in same-model review.

If only one external CLI is available, fall back to 2-model review (already supported).
If neither is available, skip gracefully.

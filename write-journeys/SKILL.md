...
description: >
  ...
  feature specs (SDD) and E2E tests (TDD). Consumes persona files from build-personas
  ...
  use build-personas instead. Mode 6 (Tiered Auto-Discovery) works from spec and personas
...
chain:
  lanes:
    greenfield: { position: 6, prev: audit-ac, next: design-ux }
    brownfield-feature: { position: 4, prev: write-spec, next: design-tech }
...
- In brownfield-feature lane: `design-tech --progressive --lane brownfield-feature`
- In drift lane: `sync-work-items --progressive --lane drift`

**If `--progressive` flag is absent:**
- Report results to user
- Suggest: "Next: consider running `design-ux`" (greenfield), "`design-tech`" (brownfield-feature), or "`sync-work-items`" (drift)

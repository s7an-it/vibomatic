...
description: >
  ...
  Wraps npx skills find/add. Invoked by domain-expert or research when they
...
**Announce at start:** "I'm using the skill-finder to search for external skills that could help."

## When Invoked

- `domain-expert` finds a domain where a specialized skill would help
- `research` can't resolve a question from internal knowledge + WebSearch
- User asks directly: "is there a skill for X?"
- During `executing-change-set` when the tech stack needs domain expertise not in `references/domains/`
...
"No external skills found for [query]. The domain-expert or research skill should build this knowledge from primary sources (docs, WebSearch)."
...
skill-finder does not participate in progressive chains.
After completing, control returns to the calling skill (domain-expert, research) or the user.

...
description: >
  ...
  "spec-ac-sync feature-learning" or "check ACs for matching", (2) all features — "spec-ac-sync" or
  "audit all specs". Triggers on: "spec-ac-sync", "check the ACs for [feature]", "audit the spec",
...
chain:
  lanes:
    greenfield: { position: 5, prev: writing-spec, next: journey-sync }
  progressive: true
  self_verify: true
...
(`spec-ac-sync`, `journey-qa-ac-testing`, `agentic-e2e-playwright`, `spec-code-sync`).
...
- **QA** — manual test status (set by `journey-qa-ac-testing`, not this skill):
  - `✅ MM-DD` = passed live
  ...
  - `—` = not yet tested
- **E2E** — automated test coverage (set by `agentic-e2e-playwright`, not this skill):
  - `✅` = covered by E2E test
...
- Invoke next skill: `journey-sync --progressive --lane greenfield`

**If `--progressive` flag is absent:**
- Report results to user
- Suggest: "Next: consider running `journey-sync`"

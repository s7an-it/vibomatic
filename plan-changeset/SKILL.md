...
chain:
  lanes:
    greenfield: { position: 12, prev: define-code-style, next: execute-changeset }
    brownfield-feature: { position: 8, prev: define-code-style, next: execute-changeset }
...
`execute-changeset` to apply directly in the branch without inventing structure.
...
- UX contradiction -> `design-ux`
- UI/design-system gap -> `design-ui`
- technical infeasibility -> `design-tech`
...
- Invoke next skill: `execute-changeset --progressive --lane <lane>`

**If `--progressive` flag is absent:**
- Report manifest summary to user
- Suggest: "Next: run `execute-changeset` to apply the plan"
